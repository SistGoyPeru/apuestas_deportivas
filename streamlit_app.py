            ...
            if len(event.selection['rows']):
                ...
                with col1:
                    ...
                    Elocal = pl.DataFrame({
                        ...
                        "Valor": [[format(df_total.TotalVictoriasEquipoLocal(liga_E, local)/df_total.TotalDisputadosEquipoLocal(liga_E, local)*100, '.2f')+"%"if df_total.TotalDisputadosEquipoLocal(liga_E, local) > 0 else "0.00%"],
                                  ...
                                  [format(df_total.liga25Local(liga_E,local)/df_total.TotalDisputadosEquipoLocal(liga_E, local)*100,'.2f')+"%" if df_total.TotalDisputadosEquipoLocal(liga_E, local) > 0 else "0.00%"],
                ...
                with col2:
                    ...
                    EVisita = pl.DataFrame({
                        ...
                        "Valor": [[format(df_total.TotalVictoriasEquipoVisita(ligas, visita)/df_total.TotalDisputadosEquipoVisita(ligas, visita)*100, '.2f')+"%" if df_total.TotalDisputadosEquipoVisita(ligas, visita) > 0 else "0.00%"],
                                  [format(df_total.TotalEmpatesEquipoVisita(ligas, visita)/df_total.TotalDisputadosEquipoVisita(ligas, visita)*100, '.2f')+"%" if df_total.TotalEmpatesEquipoLocal(ligas, local) > 0 else "0.00%"],
                                  [format(df_total.TotalPerdidasEquipoVisita(ligas, visita)/df_total.TotalDisputadosEquipoVisita(ligas, visita)*100, '.2f')+"%" if df_total.TotalDisputadosEquipoVisita(ligas, visita) > 0 else "0.00%"],
                                  [format(df_total.AEMVisita(ligas,visita)/df_total.TotalDisputadosEquipoVisita(ligas, visita)*100,'.2f')+"%" if df_total.TotalDisputadosEquipoVisita(ligas, visita) > 0 else "0.00%"],
                                  [format(df_total.liga05Visita(liga_E,visita)/df_total.TotalDisputadosEquipoVisita(liga_E, visita)*100,'.2f')+"%" if df_total.TotalDisputadosEquipoVisita(liga_E, visita) > 0 else "0.00%"],
                                  [format(df_total.liga15Visita(liga_E,visita)/df_total.TotalDisputadosEquipoVisita(liga_E, visita)*100,'.2f')+"%" if df_total.TotalDisputadosEquipoVisita(liga_E, visita) > 0 else "0.00%"],
                                  [format(df_total.liga25Visita(liga_E,visita)/df_total.TotalDisputadosEquipoVisita(liga_E, visita)*100,'.2f')+"%" if df_total.TotalDisputadosEquipoVisita(liga_E, visita) > 0 else "0.00%"],
                ...
                pronostico = pl.DataFrame({
                        ...
                        "Valor": [format(df_total.VictoriaLocal(ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.EmpateResultado(ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.VictoriaVisita(ligas, local, visita)*100, '.2f')+"%"
                ...
                pronostico = pl.DataFrame({
                        ...
                        "Valor": [format((df_total.VictoriaLocal(ligas, local, visita)+df_total.EmpateResultado(ligas, local, visita))*100, '.2f')+"%",
                                  format((df_total.VictoriaLocal(ligas, local, visita)+df_total.VictoriaVisita(ligas, local, visita))*100, '.2f')+"%",
                                  format((df_total.VictoriaVisita(ligas, local, visita)+df_total.EmpateResultado(ligas, local, visita))*100, '.2f')+"%"

                                  ],
                        
                        "Cuota": [format(1/(df_total.VictoriaLocal(ligas, local, visita)+df_total.EmpateResultado(ligas, local, visita)), '.2f'),
                                  format(1/(df_total.VictoriaLocal(ligas, local, visita)+df_total.VictoriaVisita(ligas, local, visita)), '.2f'),
                                  format(1/(df_total.VictoriaVisita(ligas, local, visita)+df_total.EmpateResultado(ligas, local, visita)), '.2f')
                                  ],
