                ...
                with col1:
                    ...
                    pronostico = pl.DataFrame({
                        ...
                        "Valor": [format(df_total.masde05goles(ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.masde15goles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.masde25goles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.masde35goles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.masde45goles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.masde55goles(
                                      ligas, local, visita)*100, '.2f')+"%"
                        ...
                        "Cuota": [format(1/(df_total.masde05goles(ligas, local, visita)), '.2f'),
                                  format(
                            1/(df_total.masde15goles(ligas, local, visita)), '.2f'),
                            format(
                                      1/(df_total.masde25goles(ligas, local, visita)), '.2f'),
                            format(
                            1/(df_total.masde35goles(ligas, local, visita)), '.2f'),
                            format(
                                      1/(df_total.masde45goles(ligas, local, visita)), '.2f'),
                            format(
                            1/(df_total.masde55goles(ligas, local, visita)), '.2f')
                ...
                with col2:
                    ...
                    pronostico = pl.DataFrame({
                        ...
                        "Valor": [format(df_total.menosde05goles(ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.menosde15goles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.menosde25goles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.menosde35goles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.menosde45goles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.menosde55goles(
                                      ligas, local, visita)*100, '.2f')+"%"
                                  ],

                        "Cuota": [format(1/(df_total.menosde05goles(ligas, local, visita)), '.2f'),
                                  format(
                            1/(df_total.menosde15goles(ligas, local, visita)), '.2f'),
                            format(
                            1/(df_total.menosde25goles(ligas, local, visita)), '.2f'),
                            format(
                            1/(df_total.menosde35goles(ligas, local, visita)), '.2f'),
                            format(
                            1/(df_total.menosde45goles(ligas, local, visita)), '.2f'),
                            format(
                            1/(df_total.menosde55goles(ligas, local, visita)), '.2f')
                        ],
                ...
                with col3:
                    pronostico = pl.DataFrame({
                        ...
                        "Valor": [format(df_total.ambosmarcan(ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.solounomarca(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.congoles(
                                      ligas, local, visita)*100, '.2f')+"%",
                                  format(df_total.cerogoles(
                                      ligas, local, visita)*100, '.2f')+"%"
                                  ],

                        "Cuota": [format(1/(df_total.ambosmarcan(ligas, local, visita)), '.2f'),
                                  format(
                            1/(df_total.solounomarca(ligas, local, visita)), '.2f'),
                            format(
                            1/(df_total.congoles(ligas, local, visita)), '.2f'),
                            format(
                            1/(df_total.cerogoles(ligas, local, visita)), '.2f')
                        ],
